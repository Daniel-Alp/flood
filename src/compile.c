...
static void compile_binary(struct Compiler *compiler, struct BinaryNode *node) 
{
    ...
    if (op_tag == TOKEN_EQ) {
        ...
        } else if (node->lhs->tag == NODE_ATTR) {
...
// FIXME this won't work once we add SET_ATTR!!!
static void compile_attr(struct Compiler *compiler, struct AttrNode *node)
{
    i32 line = node->base.span.line;
    compile_node(compiler, node->lhs);

    i32 len = node->attr.len;
    char *chars = allocate((len+1)*sizeof(char));
    memcpy(chars, node->attr.start, len);
    chars[len] = '\0';
    ...
    emit_byte(cur_chunk(compiler), OP_GET_ATTR, line);    
...
static void compile_fn_decl(struct Compiler *compiler, struct FnDeclNode *node)
{
    i32 line = node->base.span.line;
    struct Symbol* sym = &symbols(compiler)[node->id];
    // compiling top-level functions is a bit different, see compile_file 
    if (sym->idx == -1) {
        sym->idx = compiler->fn_local_cnt;
        compiler->fn_local_cnt++;
    }
...
static void compile_node(struct Compiler *compiler, struct Node *node) 
{
    ...
    case NODE_ATTR:      compile_attr(compiler, (struct AttrNode*)node); break;

...
static struct FnDeclNode *mk_fn_decl(struct Arena *arena, struct Span span, struct Span *param_spans, u32 arity, struct BlockNode *body) {
...
static struct ModuleNode *mk_module(struct Arena *arena, struct Node **stmts, u32 count) {
...
static struct Node *parse_expr(struct Parser *parser, u32 prec_lvl) {
    ...
    while(eat(parser, TOKEN_L_PAREN)) {
        ...
        struct Node **args = (struct Node **)move_ptr_array_to_arena(&parser->arena, &scratch_args);
...
static struct FnDeclNode *parse_fn_decl(struct Parser *parser) {
    ...
    struct Span *param_spans = move_span_array_to_arena(&parser->arena, &scratch_spans);
...
static struct BlockNode *parse_block(struct Parser *parser) {
    ...
    struct Node **stmts = (struct Node**)move_ptr_array_to_arena(&parser->arena, &scratch);
...
static struct ModuleNode *parse_module(struct Parser *parser) {
    ...
    u32 count = scratch.count;
    struct Node **stmts = (struct Node**)move_ptr_array_to_arena(&parser->arena, &scratch);
    return mk_module(&parser->arena, stmts, count);
}
